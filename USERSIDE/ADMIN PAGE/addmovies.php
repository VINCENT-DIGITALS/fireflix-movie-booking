...
class Movie
{
  ...
  public function insertMovieData()
  {
    ...
    $sql = "INSERT INTO movies (movie_name, synopsis, running_time, price, discount, poster, genre, releasedate, trailer, showdate)
            VALUES ('$movie_name_escaped', '$synopsis_escaped', '$this->running_time', '$this->price', '$this->discount', '$this->poster', '$this->genre','$this->releasedate', '$this->trailer', '$this->showdate')";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if ($movie->releasedate > $movie->showdate) {
    echo '<div id="my-panel" class="panel">
  <h2>Invalid Release/Show Date!</h2>
  <button id="close-btn">X</button>
  </div>';
  } elseif ($movie->releasedate <= $movie->showdate) {
...
<body>
        ...
        <div class="forminput">
          <form method="post" enctype="multipart/form-data">
            <div class="others">
              ...
              <input type="number" name="running_time"></input><br>
              ...
              <input type="number" name="discount"></input><br>

              <label>Release Date:</label>
              <input type="datetime-local" name="releasedate"></input><br>

              <label>Show Date:</label>
              <input type="datetime-local" name="showdate"></input><br>
  ...
  <script>
    ...
    const showDateInput = document.querySelector('input[name="showdate"]');

    const releaseDate = releaseDateInput.value;
    const showDate = showDateInput.value;

    if (isReleaseDateBeforeShowDate(releaseDate, showDate)) {
      // Release date is before show date
    } else {
      // Release date is after or same as show date
    }
    AOS.init();
